...
const CallComponent: React.FC<ICallBlockProps> = ({ phoneNumber }) => {
  ...
  return (
    <div className='flex gap-4 border border-gray-300 p-4 w-fit rounded-lg'>
      {isReady && !isCalling && (
      ...
      {isCalling && (
        <>
          <Button id='button-hangup-outgoing'>Hang Up</Button>
      ...
      {audioSelectionVisible && (
        <div id='output-selection' className='flex gap-4'>
          <div>
            ...
            <select
              className='inline-block font-primary text-xs rounded-xl font-semibold bg-gray-900 w-[10vw] text-white p-2'
              id='speaker-devices'
      ...
      {!isReady && (
        <div className='flex gap-5'>
          <Button onClick={startupClient}>Start Client</Button>
        </div>
      )}
    </div>
  );
